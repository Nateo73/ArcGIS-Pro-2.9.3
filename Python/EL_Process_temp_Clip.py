...
def project_ks_clip_layer():
    ...
    try:
        ...
        project_layer =  aprxMap.listLayers("Clip_ElevationCounty*")[0]
...
def move_elevation_layer_back():
    try:
        ...
        Lidar=aprxMap.listLayers("Height*")[0]
        aprxMap.moveLayer(Lidar,eleLayer,"AFTER")
...
def changenameBack():
    try:
        el_layer = "Elevation_ks"
        in_layer =  aprxMap.listLayers("Lidar_ks*")[0]
        in_layer_str = str(in_layer)
        countyFips = in_layer_str[-3:]
        elLyr = aprxMap.listLayers("ElevationCounty*")[0]
        elLyrstring = str(elLyr.name)
        elLyr.name = elLyr.name.replace(elLyrstring,el_layer + countyFips)          
...
def removeOrginClip():
    try:
        clipLayer=aprxMap.listLayers("Clip_ElevationCounty*")[0]
