...
#
#==========================================================================================
"""This script has been modified from its orginal format to work in Kansas NRCS workflows. The core of this tool is still primarly the code and work of Adolfo Diaz's.
To whom we owe a great amount of gratitude for creating this script.  """
# ==========================================================================================
# Modified 7/8/2021
...
def createOutputFC(metadata,outputWS,shape="POLYGON"):
    ...
    try:
        ...
        newFC = outputWS + os.sep + "CLU_ks" + str(adminState) + "_" + str(adminCounty) + "_" + str(tnum)
...
def getCLUgeometryByTractQuery(sqlQuery,fc,RESTurl):
    ...
    try:
        ...
        if not len(geometry['features']):
            AddMsgAndPrint("\nThere were no CLU fields associated with tract Number " + str(tnum),1)
            return False
...
def start(state,county,trctNmbr,outSR,outWS,addCLUtoSoftware=False):
    ...
    try:
        ...
        global bArcGISPro
        global adminState, adminCounty, tnum, outSpatialRef, outputWS
...
##            arcpy.env.scratchWorkspace = scratchWS
        ...
        if adminState == '02':
            whereClause = "ADMIN_STATE = " + str(adminState) + " AND COUNTY_ANSI_CODE = " + str(adminCounty) + " AND TRACT_NUMBER = " + str(tnum)
        else:
            whereClause = "ADMIN_STATE = " + str(adminState) + " AND ADMIN_COUNTY = " + str(adminCounty) + " AND TRACT_NUMBER = " + str(tnum)
        ...
        if numOfCLUs > 1:
            AddMsgAndPrint("\nThere are " + splitThousands(numOfCLUs) + " CLU fields associated with tract number " + str(tnum))
        else:
            AddMsgAndPrint("\nThere is " + splitThousands(numOfCLUs) + " CLU field associated with tract number " + str(tnum))
...
if __name__ == '__main__':
    ...
    try:
        ...
        adminCountyName = arcpy.GetParameterAsText(0)
        counties = {'Allen': '001', 'Anderson': '003', 'Atchison': '005', 'Barber': '007', 'Barton': '009', 'Bourbon': '011', 'Brown': '013', 'Butler': '015', 'Chase': '017', 'Chautauqua': '019',
                    'Cherokee': '021', 'Cheyenne': '023', 'Clark': '025', 'Clay': '027', 'Cloud': '029', 'Coffey': '031', 'Comanche': '033', 'Cowley': '035', 'Crawford': '037', 'Decatur': '039', 'Dickinson': '041',
                    'Doniphan': '043', 'Douglas': '045', 'Edwards': '047', 'Elk': '049', 'Ellis': '051', 'Ellsworth': '053', 'Finney': '055', 'Ford': '057', 'Franklin': '059', 'Geary': '061', 'Gove': '063',
                    'Graham': '065', 'Grant': '067', 'Gray': '069', 'Greeley': '071', 'Greenwood': '073', 'Hamilton': '075', 'Harper': '077', 'Harvey': '079', 'Haskell': '081', 'Hodgeman': '083', 'Jackson': '085',
                    'Jefferson': '087', 'Jewell': '089', 'Johnson': '091', 'Kearny': '093', 'Kingman': '095', 'Kiowa': '097', 'Labette': '099', 'Lane': '101', 'Leavenworth': '103', 'Lincoln': '105', 'Linn': '107',
                    'Logan': '109', 'Lyon': '111', 'Marion': '115', 'Marshall': '117', 'McPherson': '113', 'Meade': '119', 'Miami': '121', 'Mitchell': '123', 'Montgomery': '125', 'Morris': '127', 'Morton': '129',
                    'Nemaha': '131', 'Neosho': '133', 'Ness': '135', 'Norton': '137', 'Osage': '139', 'Osborne': '141', 'Ottawa': '143', 'Pawnee': '145', 'Phillips': '147', 'Pottawatomie': '149', 'Pratt': '151',
                    'Rawlins': '153', 'Reno': '155', 'Republic': '157', 'Rice': '159', 'Riley': '161', 'Rooks': '163', 'Rush': '165', 'Russell': '167', 'Saline': '169', 'Scott': '171', 'Sedgwick': '173',
                    'Seward': '175', 'Shawnee': '177', 'Sheridan': '179', 'Sherman': '181', 'Smith': '183', 'Stafford': '185', 'Stanton': '187', 'Stevens': '189', 'Sumner': '191', 'Thomas': '193', 'Trego': '195',
                    'Wabaunsee': '197', 'Wallace': '199', 'Washington': '201', 'Wichita': '203', 'Wilson': '205', 'Woodson': '207', 'Wyandotte': '209'}
        adminCounty = counties[adminCountyName]
        zone_15 = ["Brown","Doniphan","Atchison","Jackson","Jefferson","Leavenworth","Wayndotte","Shawnee","Douglas","Johnson","Osage","Franklin","Miami","Coffey","Anderson","Linn",
                   "Woodson","Alle","Bourbon","Wilson","Neosho","Crawford","Montgomery","Labette","Cherokee"]
        tractNumber = arcpy.GetParameter(1)
        tractNumber_no_space = tractNumber.replace(" ","")
        tract_num_list = tractNumber_no_space.split(",")
        if adminCountyName in zone_15:
            outSpatialRef = 'PROJCS["NAD_1983_UTM_Zone_15N",GEOGCS["GCS_North_American_1983",DATUM["D_North_American_1983",SPHEROID["GRS_1980",6378137.0,298.257222101]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Transverse_Mercator"],PARAMETER["False_Easting",500000.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",-93.0],PARAMETER["Scale_Factor",0.9996],PARAMETER["Latitude_Of_Origin",0.0],UNIT["Meter",1.0]];-5120900 -9998100 10000;-100000 10000;-100000 10000;0.001;0.001;0.001;IsHighPrecision'
        else:
            outSpatialRef = 'PROJCS["NAD_1983_UTM_Zone_14N",GEOGCS["GCS_North_American_1983",DATUM["D_North_American_1983",SPHEROID["GRS_1980",6378137.0,298.257222101]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Transverse_Mercator"],PARAMETER["False_Easting",500000.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",-99.0],PARAMETER["Scale_Factor",0.9996],PARAMETER["Latitude_Of_Origin",0.0],UNIT["Meter",1.0]];-5120900 -9998100 10000;-100000 10000;-100000 10000;0.001;0.001;0.001;IsHighPrecision'
        outputWS = r"C:\geodata\project_data\nrcs\ArcGIS_Pro\Geodatabases\KS_scripting_ws.gdb"
        addToSoftware = True
...
##        addToSoftware = False
        for tract in tract_num_list:
            tnum = tract
            start(adminState,adminCounty,tnum,outSpatialRef,outputWS,addToSoftware)
